    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cyber-laser-scan {
            ...
            box-shadow: 0 0 18px #00f3ff, 0 0 35px #ff007f;
            z-index: 999998;
            pointer-events: none;
            opacity: 0.8;
            animation: cyberLaserScan 5s linear infinite;
        }
        ...
        /* ── INTERACTIVE 3D TILT EFFECT FOR HERO, CARDS & TERMINAL ── */
        .shield-hero, .terminal-box, .stat-box, .week-preview-card {
...
<!-- REPRODUCTOR DE MUSICA CYBERPUNK REAL (LOUD & CLEAR) -->
...
function playCyberMusic() {
    ...
    cyberAudio.volume = 0.70; // Volumen audible y nítido
    const p = cyberAudio.play();
    if (p !== undefined) {
        p.then(() => {
            syncMusicState(true);
        }).catch(err => {
...
/* Desbloqueo automático garantizado al primer clic en cualquier zona */
...
/* ── 3D TILT INTERACTIVO EXTRAVAGANTE ─────────────────── */
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.shield-hero, .terminal-box, .stat-box, .week-preview-card').forEach(elem => {
        elem.addEventListener('mousemove', e => {
            ...
            elem.style.transform = perspective(900px) rotateX(deg) rotateY(deg) scale3d(1.025, 1.025, 1.025);
            elem.style.boxShadow =   18px 45px rgba(0, 243, 255, 0.45);
        });
        elem.addEventListener('mouseleave', () => {
            elem.style.transform = 'perspective(900px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1)';

    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cyber-fx-pill {
            ...
            cursor: pointer;
            user-select: none;
            box-shadow: 0 0 16px rgba(0, 243, 255, 0.4), inset 0 0 12px rgba(0, 243, 255, 0.12);
        ...
        .cyber-fx-pill:hover {
            ...
            border-color: #ff007f;
            box-shadow: 0 0 25px rgba(255, 0, 127, 0.6), inset 0 0 15px rgba(255, 0, 127, 0.2);
        }
        .cyber-fx-wheel {
            width: 22px;
            height: 22px;
        ...
        .cyber-fx-pill.playing .eq-teal {
            animation: eqAnim1 0.7s ease-in-out infinite alternate;
        }
        .cyber-fx-pill.playing .eq-pink {
            animation: eqAnim2 0.5s ease-in-out infinite alternate;
        }
        .cyber-fx-pill.playing .eq-purple {
            animation: eqAnim3 0.65s ease-in-out infinite alternate;
        }
        .cyber-fx-pill.playing .eq-yellow {
            animation: eqAnim4 0.45s ease-in-out infinite alternate;
        }

        @keyframes eqAnim1 { 0% { height: 5px; } 100% { height: 18px; } }
        @keyframes eqAnim2 { 0% { height: 16px; } 100% { height: 4px; } }
        @keyframes eqAnim3 { 0% { height: 4px; } 100% { height: 17px; } }
        @keyframes eqAnim4 { 0% { height: 18px; } 100% { height: 7px; } }

        /* Banner flotante para desbloquear audio en navegadores */
        .audio-hint {
            position: fixed;
            bottom: 24px;
            left: 50%;
            ...
            animation: fadeInUp 0.5s ease, borderGlow 3s infinite;
            cursor: pointer;
...
<header class="hud-top">
    ...
    <div class="hud-stats">
        ...
        <div class="cyber-fx-pill" id="cyberFxBtn" onclick="toggleCyberMusic(event)" title="Haz clic para activar o pausar la música cósmica">
        ...
        <a href="<%= ctx %>/login" class="btn-hud-in">&#9889; INGRESAR</a>
    </div>
</header>
...
<section class="hero-section">
    ...
    <%-- Botones de acción --%>
    <div class="btn-group">
        <a href="<%= ctx %>/login" class="btn-primary">
            &#128274; ACCEDER AL PORTAFOLIO
        </a>
        <a href="<%= ctx %>/registro" class="btn-secondary">
            &#43; CREAR CUENTA
        </a>
    </div>
...
function updateCyberFps() {
    ...
    if (delta >= 250) {
        const fps = Math.min(Math.round((frameCounter * 1000) / delta), 120);
...
/* ── MOTOR DE AUDIO CÓSMICO / CYBER FX SYNTH ENGINE ──── */
let audioCtx = null;
let masterGain = null;
let filterNode = null;
...
/* Notas de la escala cósmica Cyber (Am / C pentatónica futurista) */
const CHORDS = [
    { name: "Am9",  notes: [220.00, 261.63, 329.63, 493.88], bass: 55.00  }, // A2, C3, E3, B3 + A1
    { name: "Fmaj7",notes: [174.61, 220.00, 261.63, 329.63], bass: 43.65  }, // F2, A2, C3, E3 + F1
    { name: "Cadd9",notes: [130.81, 196.00, 261.63, 293.66], bass: 65.41  }, // C2, G2, C3, D3 + C1
    { name: "Em7",  notes: [164.81, 196.00, 246.94, 293.66], bass: 41.20  }  // E2, G2, B2, D3 + E1
];
const ARP_NOTES = [440, 523.25, 659.25, 783.99, 880, 987.77, 1046.50, 1318.51];
...
/* Toca una nota sintetizada suave tipo pad cósmico */
function playSynthNote(freq, startTime, duration, vol, type = 'sawtooth') {
    ...
    try {
        ...
        noteGain.gain.linearRampToValueAtTime(vol, startTime + 0.4);
...
/* Bajo sub-grave cibernético */
function playSubBass(freq, startTime, duration) {
    if (!audioCtx || !masterGain) return;
    try {
        const osc = audioCtx.createOscillator();
        ...
        bGain.gain.setValueAtTime(0.001, startTime);
        bGain.gain.linearRampToValueAtTime(0.35, startTime + 0.15);
        bGain.gain.exponentialRampToValueAtTime(0.0001, startTime + duration);
...
/* Ciclo musical continuo procedural */
function scheduleMusicLoop() {
    if (!isAudioActive || !audioCtx) return;
    
    const now = audioCtx.currentTime;
    const chord = CHORDS[currentChordIdx];
    const chordDuration = 3.6; // Segundos por acorde
    
    // Tocar acordes atmosféricos espaciales
    chord.notes.forEach((f, idx) => {
        playSynthNote(f, now + 0.05, chordDuration, 0.08, 'sawtooth');
        playSynthNote(f * 1.003, now + 0.05, chordDuration, 0.05, 'triangle');
    });
    
    // Sub-bass
    playSubBass(chord.bass, now + 0.05, chordDuration * 0.95);
    
    // Arpegios cósmicos de fondo
    const arpSteps = 8;
    const stepDuration = chordDuration / arpSteps;
    for (let s = 0; s < arpSteps; s++) {
        const note = ARP_NOTES[(arpStep + s * 2) % ARP_NOTES.length];
        const t = now + s * stepDuration;
        playSynthNote(note, t, stepDuration * 0.8, 0.04, 'sine');
    }
    arpStep = (arpStep + 3) % ARP_NOTES.length;
    currentChordIdx = (currentChordIdx + 1) % CHORDS.length;
    
    // Agendar siguiente acorde
    synthTimer = setTimeout(scheduleMusicLoop, (chordDuration - 0.2) * 1000);
}

/* Efecto sonoro Cyber al interactuar con botones */
function playCyberBeep(freq = 880) {
...
/* Iniciar / Activar Audio */
function startCyberAudio(userInitiated = false) {
    initAudioContext();
    ...
    if (!isAudioActive) {
        ...
        const pill = document.getElementById('cyberFxBtn');
        if (pill) pill.classList.add('playing');
        
        scheduleMusicLoop();
    }
    
    // Ocultar banner flotante de inicio
    const hint = document.getElementById('audioHint');
    if (hint) hint.classList.add('hide');
}

/* Alternar Play / Pause desde el botón CYBER FX */
...
/* Desbloqueo automático al primer clic en cualquier parte de la página */
window.addEventListener('click', () => {
    if (!isAudioActive) {
        startCyberAudio(false);
    }
}, { once: true });

/* Sonidos futuristas al pasar por botones clave */
document.addEventListener('DOMContentLoaded', () => {
    const buttons = document.querySelectorAll('.btn-primary, .btn-secondary, .btn-hud-in, .week-preview-card');
    buttons.forEach(btn => {
        btn.addEventListener('mouseenter', () => playCyberBeep(700));
        btn.addEventListener('click', () => playCyberBeep(1100));
    });
});
</script>

<%-- ══ BANNER FLOTANTE DE DESBLOQUEO DE AUDIO ═══════════════ --%>
<div class="audio-hint" id="audioHint" onclick="startCyberAudio(true)">
    <i class="fas fa-volume-up" style="color:#00f3ff; font-size:14px;"></i>
    <span><b>CYBER FX AUDIO</b> &bull; Haz clic aquí para activar el sonido cósmico</span>
    <span style="background:rgba(0,243,255,0.2); border:1px solid #00f3ff; border-radius:12px; padding:3px 10px; color:#00f3ff; font-size:10px; font-weight:800;">ACTUALIZAR / ON</span>
</div>
